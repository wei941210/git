...
public interface PutinPointCountMapper {
    ...
    // 获取 各点位 统计 排名
    @Select("<script>" +
            "select h.screenName as screenName  ,g.screenCutName as screenCutName ,count(*) as strategyCount  " +
            ...
            "where year(a.playTime) =${year} group by h.screenName ,g.screenCutName order by strategyCount desc  "+
            "</script>")
    ...
    @Select("<script>" +
            " select e.screenName as screenName,f.screenCutName as screenCutName ,count(*) as strategyCount   " +
            "from ad.t_advert a  " +
            "inner join ad.t_sub_advert_policys b on b.advertId = a.advertId " +
            ...
            "where year(a.uploadTime) = ${year}  group by e.screenName,f.screenCutName order by strategyCount desc  " +
            "</script>")
    ...
    @Select("<script>" +
            ...
            "inner join ad.t_screen_cut f on f.screenCutId = d.screenCutId "+
            "where year(a.playTime) = ${year} " +
            "group by e.screenName,f.screenCutName " +
            "</script>")
