...
public class DownloadToZipCtrl {
    ...
    public MessageModel downloadZip(String fileNames,String dirPath){
        ...
        //1、将需要下载的目录清空
        emptyFolder(advertfiles+File.separator+dirPath);
...
//            downloadZipToServer("downloadFiles.zip",response);
    ...
    public void emptyFolder(String path){
        ...
        if(f.exists()){
            ...
            for (int i = 0; i < lists.length; i++) {
                File file=new File(path+File.separator+lists[i]);
                if(file.isDirectory()){
    ...
    public void downloadFileToZip(String dirPath,String fileName){
        //获取文件源文件路径
        String respath=advertfiles+"media"+File.separator;
        //设置下载文件路径
        String path=advertfiles+File.separator+dirPath;
        File f=new File(path);
        if(!f.exists()){
            f.mkdirs();
        }
    ...
    public void compressFilesToZip(String path, File file, ZipOutputStream out) throws IOException {
        ...
        if (file.isDirectory()) {
            ...
            //将文件夹添加到下一级打包目录
            out.putNextEntry(new ZipEntry(path + File.separator));
