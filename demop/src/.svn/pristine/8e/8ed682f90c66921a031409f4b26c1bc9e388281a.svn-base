...
public interface AdReplaceMapper {
    ...
    //更新 广告状态表
    @Insert(
            " UPSERT into ad.t_advert(advertId,nowState) " +
            " SELECT advertId,TO_NUMBER('${nowState}') as nowState from ad.t_advert WHERE advertId='${advertId}' ")
    ...
    @Select(
            ...
            " from ad.t_sub_advert_policys as a left join ad.t_advert_media as b on a.advertId=b.advertId  " +
...
/*@ysh 因为idcard 与 ad 不一定在一个服务器，所以避免使用idcard库 20190326*/
...
//            " from ad.t_fileHost as a  inner join idcard.t_device as b on a.cityId = b.planCityId and b.deviceId in ('${devices}')")
    ...
    @Insert(
            ...
            " deviceId,  " +
            " browseTime, " +
            " regexp_replace(mediaFiles, '${oldFileName}', '${newFileName}')  as  mediaFiles, " +
