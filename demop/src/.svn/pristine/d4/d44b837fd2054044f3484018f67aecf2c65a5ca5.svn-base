...
public class AdvertCtrl {
    ...
    public MessageModel advertList(String state , String advertName,String advertId,String tradeId,String adCorpName,String blankId ,String uploadTimeBegin,
    ...
    @ApiOperation( value = "获取文件加密路径外部接口",notes = "外调接口：获取加密文件路径" )
    @GetMapping(value = "/getFilePath")
    public MessageModel getFilePath(String fileName){
        MessageModel model=new MessageModel();
        Map<String,Object> map=new HashMap<String, Object>();
        map.put("fileName",fileName);
        String advertpath=svr.getFilePath(map);
        model.setResultCode("1");
        model.setData(advertpath);
        return model;
    }
