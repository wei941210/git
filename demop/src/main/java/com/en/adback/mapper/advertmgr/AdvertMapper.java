...
//    ad.nowState
   ...
   public List<Map<String,Object>> advertAuditCount(Map<String, Object> map);
    ...
    // 插入广告媒体表
    @Insert("upsert into ad.t_advert_media(downLoadFileName,advertId,fileName,fileSize,duration,downLoadUrl,fileMD5) values(" +
            " #{downLoadFileName},#{advertId},#{fileName},#{fileSize},#{duration},#{downLoadUrl},#{fileMD5} )")
    ...
    @Select( "select fileMd5 from ad.t_advert_media where downloadFileName='${fileName}'\n" +
            "union all\n" +
            "select fileMd5 from ad.t_defaultAdvert where defaultFileName='${fileName}'")
    List<Map<String,Object>> getFilePath(Map<String,Object> map);
}
