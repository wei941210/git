...
public class DeviceRedis {
    ...
    public List<DeviceCutAdvert>  scrollExistDeviceIds(String deviceIds,String playDates,String screenId, String screenCutId,String advertId ){
        ...
        for(String thedate : playDates.split(",")) {
            Set<String> keys = redisTemplate.keys("*_"+thedate+"_*" );
            Arrays.stream(deviceIds.split(",")).forEach(did -> {
                        for (String key : keys)
                        {
                              if (key.contains(did+"_") && key.contains("_" +screenId + "_"+screenCutId ) && key.contains("_1_") && !key.contains(advertId))
                              {
...
//
//        Arrays.stream(deviceIds.split(",")).forEach(did -> {
//                    Set<String> keys = redisTemplate.keys(did+"*" );
//                    for(String thedate : playDates.split(",")) {
//
//                        for (String key : keys)
//                        {
//                            if (key.contains("_" +thedate+"_" +screenId + "_"+screenCutId ) && key.contains("_1_") && !key.contains(advertId))
//                            {
//                                list.add(new DeviceCutAdvert(key));
//                            }
//                        }
    ...
    public List<DeviceCutAdvert>  aloneExistDeviceIds(String deviceIds,String playDates,String screenId, String screenCutId,String advertId ){
        ...
        for(String thedate : playDates.split(",")) {
            ...
            Arrays.stream(deviceIds.split(",")).forEach(did -> {
                        for (String key : keys)
                        {
                            if (key.contains(did+"_") && key.contains("_" +screenId + "_"+screenCutId ) && key.contains("_2_") && !key.contains(advertId))
                            {
                                list.add(new DeviceCutAdvert(key));
                            }
                        }
...
//                        for (String key : keys)
//                        {
//                            if (key.contains("_" +thedate+ "_" +screenId + "_"+screenCutId ) && key.contains("_2_") && !key.contains(advertId))
//                            {
//                                list.add(new DeviceCutAdvert(key));
//                            }
//                        }
            ...
            Arrays.stream(deviceIds.split(",")).forEach(did -> {
                ...
                for(String thedate : playDates.split(",")) {
                    //    Set<String> keys = redisTemplate.keys(did +"_" +thedate+ "_" +screenId + "_"+screenCutId+"*");
                        ...
                        for (String key : keys)
                        {
                            if (key.contains( "_" +thedate+ "_") && !key.contains(advertId))
                            {
